...
//	private ImageService imageService;
	...
	@PutMapping("/{id}")
	public ResponseEntity<UsuarioDTO> alterar(@PathVariable Long id, @Valid @RequestBody UsuarioAlterarDTO usuarioAlterar) {
		if (!usuarioRepository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
		Usuario usuarioTemp = usuarioService.findById(id);
		usuarioAlterar.setId(id);
		UsuarioDTO usuarioDTO = usuarioService.alterar(usuarioAlterar, usuarioTemp);
